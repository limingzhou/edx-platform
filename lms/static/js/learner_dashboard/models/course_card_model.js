...
class CourseCardModel extends Backbone.Model {
  ...
  formatDateString(run) {
    ...
    let dateString;
    const start = this.valueIsDefined(run.start_date) ? run.advertised_start || run.start_date :
      this.get('start_date');
    const end = this.valueIsDefined(run.end_date) ? run.end_date : this.get('end_date');
    const now = new Date();
  ...
  setUnselected() {
    ...
    const unselectedCourseRun = this.getUnselectedCourseRun(this.get('enrollable_course_runs'));
...
export { CourseCardModel as default };
